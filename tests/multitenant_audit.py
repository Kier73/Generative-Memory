...
def run_multitenant_audit():
    print("=== GVM Phase 15: Multi-Tenant & Quota Audit ===")
    
    with open("src/multitenant_audit.c", "w") as f:
        f.write(f"""
#include <windows.h>
#include <stdio.h>
#include <stdlib.h>
...
""")
    ...
    print("  Compiling Audit Utility...")
    subprocess.run(["gcc", "-O3", "-mavx2", "-Iinclude", "-Isrc", "src/gmem.c", "src/gmem_alloc.c", "src/gmem_filter_json.c", "src/gmem_filter_block.c", "src/gmem_vrns.c", "src/gmem_trinity.c", "src/gmem_aro.c", "src/gmem_manager.c", "src/multitenant_audit.c", "-o", "build/Release/multitenant_audit.exe"], check=True)
    
    # Run
    result = subprocess.run(["./build/Release/multitenant_audit.exe"], capture_output=True, text=True)
    print(result.stdout)
    if result.returncode == 0:
        print("[SUCCESS] Phase 15 Verified: Hypervisor Isolation & Quotas active.")
    else:
        print(f"[ERROR] Audit Failed with Exit Code {result.returncode}")
        print(result.stderr)

if __name__ == "__main__":
    run_multitenant_audit()
